"""
v6.1 PE-Band Strategy -- PE Spread CSI300/CSI500 Rotation
==========================================================
PE-band equity allocation + gold trend filter. When CSI300 PE is
significantly higher than CSI500 PE, rotates a portion of equity
from CSI300 to CSI500.

Enhancement over v6.0:
  - CSI500 PE data (fetched from akshare or synthetic fallback)
  - CSI300/CSI500 PE spread rotation (1.25 threshold, 50% rotation)
  - Allocation split: CSI300 and CSI500 from equity pool
  - Original trend() used for gold (not trend_robust)
  - No bond trend filter (bonds always receive residual)

No data fetching, no health checks, no CLI boilerplate.
Operational layer is run_daily.py.

Run:
    python scripts/v6_pe_band_v6_1.py               # Walk-forward + diagnostics + benchmarks
    python scripts/v6_pe_band_v6_1.py --verify       # 19-criterion acceptance test
    python scripts/v6_pe_band_v6_1.py --diagnostics  # Diagnostic year tests
    python scripts/v6_pe_band_v6_1.py --benchmarks   # Benchmark comparison
"""
...
CASH, GOLD, BOND, CSI300, CSI500 = "511880", "518880", "511010", "510300", "510500"
ROLLING_DAYS = 5 * 252

# ─── PE spread config (FIXED, not grid-searched) ───
PE_SPREAD_THRESHOLD = 1.25
CSI500_FRACTION = 0.50
...
def reload_data():
    """(Re)load ETF, CSI300 PE, and CSI500 PE data from disk."""
    global T, pe_raw, pe_raw500
    T = {}
    for e in ETFS:
        fp = DIR / f"{e}.parquet"
        if not fp.exists():
    ...
    # ─── CSI500 PE ───
    pe500_path = MACRO / "csi500_pe.parquet"
    if pe500_path.exists():
        pe_raw500 = pd.read_parquet(pe500_path).set_index("date")
        pe_raw500["pe_pct"] = np.nan
        for i in range(ROLLING_DAYS, len(pe_raw500)):
            window = pe_raw500["pe"].iloc[max(0, i - ROLLING_DAYS):i]
            pe_raw500.iloc[i, pe_raw500.columns.get_loc("pe_pct")] = (
                (window <= pe_raw500["pe"].iloc[i]).sum() / max(len(window), 1)
            )
    else:
        # Synthetic fallback: CSI500 PE is typically 1.1-1.5x CSI300 PE
        # (small-caps trade at a premium). When akshare symbol 中证500 is not
        # available, use CSI300 PE * 1.3 as a reasonable approximation.
        if pe_raw is not None:
            pe_raw500 = pe_raw[["pe"]].copy()
            pe_raw500["pe"] *= 1.3
            pe_raw500["pe_pct"] = pe_raw["pe_pct"].copy()
        else:
            pe_raw500 = pd.DataFrame({"pe": [19.5]}, index=[pd.Timestamp("2024-01-01")])
            pe_raw500["pe_pct"] = 0.5
...
def pe_pct_at_500(dt):
    """CSI500 PE 5-year percentile on date (PIT)."""
    try:
        r = pe_raw500.loc[:pd.Timestamp(dt)].iloc[-1]
        pct = float(r["pe_pct"])
        return pct if not pd.isna(pct) else 0.5
    except: return 0.5

def pe_spread(dt):
    """CSI300 PE / CSI500 PE ratio. >= PE_SPREAD_THRESHOLD triggers rotation."""
    try:
        r300 = pe_raw.loc[:pd.Timestamp(dt)].iloc[-1]
        r500 = pe_raw500.loc[:pd.Timestamp(dt)].iloc[-1]
        pe300 = float(r300["pe"])
        pe500 = float(r500["pe"])
        return pe300 / max(pe500, 1e-6)
    except:
        return 0.0
...
def backtest(start, end, eq_max=0.60, eq_min=0.10, gold_max=0.30, gold_ma=50,
    ...
    """PE-band + gold trend + PE spread CSI300/CSI500 rotation.
    4-ETF portfolio (CSI300, CSI500, Gold, Bonds), monthly rebalancing."""
    ...
    for i, d in enumerate(dr):
        if i == 0 or d.month != dr[i - 1].month:
            ...
            # CSI300/CSI500 PE spread rotation
            spread = pe_spread(d)
            csi500_fraction = CSI500_FRACTION if spread >= PE_SPREAD_THRESHOLD else 0.0

            eq300_pct = eq_pct * (1.0 - csi500_fraction)
            eq500_pct = eq_pct * csi500_fraction

            g_pct = gold_max if trend(GOLD, d, gold_ma) else 0.0
            bd_pct = max(0.0, 1.0 - eq300_pct - eq500_pct - g_pct)
            hh = {}
            for etf, pct in [(CSI300, eq300_pct), (CSI500, eq500_pct),
                              (GOLD, g_pct), (BOND, bd_pct)]:
                if pct > 0.005:
...
def run_walk_forward():
    ...
    PE spread threshold (1.25) and rotation fraction (0.50) are FIXED, not grid-searched."""
    print("=" * 70); print("v6.1 PE-Band -- 3-Fold Walk-Forward (2015-2025)"); print("=" * 70)
    oos_rets = []
...
def run_verify():
    """19-criterion acceptance test (17 v6.0 criteria + 2 new v6.1 AC-18/19)."""
    print("="*70); print("v6.1 PE-Band -- 19-Criterion Acceptance Test"); print("="*70)
    ...
    # CSI500 allocation tracking
    csi500_wts = []
    for i, h in enumerate(bt_full["held"]):
        dt = bt_full["r"].index[i]; pv = (1 + bt_full["r"].fillna(0)).cumprod().iloc[i]
        c5_val = h.get(CSI500, 0) * float(T[CSI500]["close"].loc[:dt].iloc[-1]) if CSI500 in h else 0
        csi500_wts.append(c5_val / pv if pv > 0 else 0)
    avg_csi500_wt = np.mean(csi500_wts) if csi500_wts else 0.0
    max_csi500_wt = max(csi500_wts) if csi500_wts else 0.0
    ...
    ac = [
        ...
        ("AC-18", "CSI500 alloc>5%",   "OOS","P0", avg_csi500_wt>0.05,    f"{avg_csi500_wt:.2%} avg"," >5% avg"),
        ("AC-19", "CSI500 frac<100%",  "OOS","P0", max_csi500_wt<1.0,     f"{max_csi500_wt:.2%} max","<100% max"),
    ]
    ...
    print("-" * 106)
    print(f"\nSummary: {n_pass} PASS, {n_fail} FAIL (of {n_pass+n_fail} applicable), {n_info} INFO, {n_na} N/A (total 19)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="v6.1 PE-Band Strategy -- PE Spread CSI300/CSI500 Rotation")
