"""Run full-period backtest and report CAGR."""
...
storage = DataStorage()
ETF_UNIVERSE = ['510300','510500','159915','510880','518880','511880']
etf_data = {}
...
def score(code, date_str):
    ...
    if len(c2) >= 28:
        ...
        dx = abs(ps-ms)/(ps+ms+1e-10)*100
        adx_val = min(dx/50, 1.0)
    else:
        adx_val = 0.5
    # momentum
    ...
    # rising MA
    ma_now = np.mean(c2[-120:])
    ma_20d = np.mean(c2[-140:-20]) if len(c2)>=140 else ma_now-0.01
    rising = ma_now > ma_20d
    if not rising: return 0
    return 0.35*trend + 0.40*adx_val + 0.25*mom

def get_monthly(dates, start, end):
    m = [d for d in dates if start<=d<=end]
    mo = []
    for d in m:
        dm = d[4:6]
        if not mo or dm!=mo[-1][4:6]: mo.append(d)
    return mo

PERIODS = [
    ('Full(2012-2026)','20120101','20260612'),
...
for pname,ps,pe in PERIODS:
    ...
    for rd in rebal:
        ...
        scored=[(c,score(c,rd)) for c in ETF_UNIVERSE if score(c,rd)>0]
        scored.sort(key=lambda x:x[1],reverse=True)
        sel={s[0] for s in scored[:3]}
        equity_ok = all(any(s[0]==e for s in scored) for e in ['510300','510500','159915'])
        if equity_ok and not (sel & {'510300','510500','159915'}):
            best_eq = max((s for s in scored if s[0] in {'510300','510500','159915'}), key=lambda x:x[1])
            worst = min((s for s in scored[:3] if s[0] in sel), key=lambda x:x[1])
            sel.discard(worst[0]); sel.add(best_eq[0])
        for sym in list(hld.keys()):
            if sym not in sel:
                p=price(sym,rd) or hld[sym]['p']; cash+=hld[sym]['q']*p*(1-COMM); del hld[sym]
    ...
    eq=np.array(eq)
    ny = (int(pe[:4])-int(ps[:4])) + (int(pe[4:6])-int(ps[4:6]))/12.0
    cagr = ((eq[-1]/eq[0])**(1/max(ny,0.5))-1)*100 if eq[0]>0 else 0
    ...
    print(f'{pname:<20s} | CAGR={cagr:+6.1f}% | Total={total_ret:+7.1f}% | Sharpe={sh:6.3f} | MaxDD={md:5.1f}% | Final={eq[-1]:>10,.0f} ({ny:.1f}y)')
