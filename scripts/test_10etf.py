"""Mid-size pool: 10 ETFs covering broad + key sectors + defense."""
...
# Mid-size pool: 宽基3 + 科技1 + 消费1 + 医药1 + 军工1 + 红利1 + 黄金1 + 货币1 = 10只
POOL = [
    "510300",  # 沪深300
    "510500",  # 中证500
    "159915",  # 创业板
...
for sym in POOL:
    ...
    if raw:
        ...
        print(f"{sym}: {len(closes)} bars ({dates[0]}~{dates[-1]})")
...
def run(start,end,top_n=3):
    ...
    for v in eq:
        ...
        d=(pk2-v)/pk2*100
        if d>md:md=d
    return cagr,sh,md,ny

PERIODS=[
    ("全历史(2015-26)","20150101","20260612"),
...
print(f"\n10-ETF池: {POOL}")
print(f"{'时期':<20s} | {'CAGR':>7s} | {'Sharpe':>7s} | {'MaxDD':>6s}")
print("-"*50)
for label,ps,pe in PERIODS:
    cagr,sh,md,ny=run(ps,pe)
    print(f"{label:<20s} | {cagr:+6.1f}% | {sh:7.3f} | {md:6.1f}%")
