...
def run_gold_sharp_decay(state_map, tc, stock_data, all_dates, bond_cl, gold_cl, start, end,
                          ...
                          bond_pct=0.80, gold_pct=0.20):
    """Gold defensive (bond/gold split) + Sharp3pct exit + A43 decay — all combined."""
    rebal = monthly_dates(all_dates, start, end)
    cash = CAPITAL; holdings = {}; bu = 0; gu = 0; eq = [cash]; max_eq = cash; dd_active = False
    months_in_cycle = 0; prev_mst = 0; genuine_prev = 0; sharp_cd = -1
    for rd in rebal:
        ...
        if sharp_fired:
            ...
            sharp_cd = ci + M_COOLDOWN; eq.append(cash + bu*(bp or 1.0) + gu*(gp or 1.0)); continue
        emst = 3 if (ci is not None and ci >= 0 and ci < sharp_cd) else mst
        if emst in (2, 4):
            if genuine_prev not in (2, 4): months_in_cycle = 1
            else: months_in_cycle += 1
...
if __name__ == "__main__":
    ...
    # Gold + Sharp3pct merged
    gs43_tr = run_gold_sharp_decay(sm_all, tc_train, stock_data, all_dates, bond_cl, gold_cl, TRAIN_START, TRAIN_END, a43_fn, csi_ret5, csi_idx_map)
    gs43_te = run_gold_sharp_decay(sm_all, tc_test,  stock_data, all_dates, bond_cl, gold_cl, TEST_START,  TEST_END, a43_fn, csi_ret5, csi_idx_map)
    print(f"\n{'='*100}")
    print("FINAL RESULTS")
    print(f"{'='*100}")
    ...
    for name, tr, te in [
        ...
        ("+ Gold+Sharp+A43 merged (80/20)",     gs43_tr, gs43_te),
    ]:
        print(f"{name:<40s} | {tr['cagr']:+7.2f}% {tr['sharpe']:6.3f} {tr['maxdd']:5.1f}% | {te['cagr']:+7.2f}% {te['sharpe']:6.3f} {te['maxdd']:5.1f}%")
    # ── Gold ratio sweep (as individual rows) ──
    g_ratio_results = {}
    for bpct, gpct, label in [(100,0,"A43+Gold(100/0)"),(90,10,"A43+Gold(90/10)"),(80,20,"A43+Gold(80/20)"),
                               (70,30,"A43+Gold(70/30)"),(60,40,"A43+Gold(60/40)"),(50,50,"A43+Gold(50/50)"),
                               (40,60,"A43+Gold(40/60)")]:
        r = run_gold_decay(sm_all, tc_test, stock_data, all_dates, bond_cl, gold_cl, TEST_START, TEST_END, a43_fn,
                          bond_pct=bpct/100, gold_pct=gpct/100)
        g_ratio_results[label] = r
        # Just print the test C for each
        print(f"{label:<40s} | {'':>7s} {'':>7s} {'':>7s} | {r['cagr']:+7.2f}% {r['sharpe']:6.3f} {r['maxdd']:5.1f}%  (test-only)")
    ...
    print(f"  +Gold+Sharp+A43 merged:  {gs43_te['cagr']-br_te['cagr']:+.2f}%")
    best_g = max(g_ratio_results.items(), key=lambda x: x[1]['cagr'])
    print(f"\n  Best gold ratio: {best_g[0]} → Test C={best_g[1]['cagr']:+.2f}% S={best_g[1]['sharpe']:.3f}")
