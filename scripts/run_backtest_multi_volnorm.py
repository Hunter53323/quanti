"""Multi-industry ETF rotation backtest with volatility normalization.

Phase 2.2 validation: tests whether ADX normalization reduces
structural bias toward high-volatility ETFs (semiconductors, new energy)
over low-volatility ones (bonds, gold).

Copied from run_backtest_multi_fast.py with VOL_NORM flag added.
"""
...
VOL_NORM = True  # When True, normalize ADX by 60-day realized vol
...
def compute_score(cl, hi, lo, vol_norm=VOL_NORM):
    m120 = sma(cl, 120)
    trend = 1.0 if (m120 is not None and not np.isnan(m120[-1]) and cl[-1] > m120[-1]) else 0.0
    if len(cl) >= 28:
        adx_v = compute_adx(hi, lo, cl)
        if vol_norm and len(cl) >= 61:
            log_ret = np.diff(np.log(cl[-61:]))
            vol_60d = np.std(log_ret) * np.sqrt(252)  # annualized
            adx_norm = max(50.0, vol_60d * 100)
            adx_val = min(adx_v / adx_norm, 1.0) if not np.isnan(adx_v) else 0.5
        else:
...
def run_backtest(progressive=True, symbols_override=None, vol_norm=VOL_NORM):
    storage = DataStorage()
    ...
    for ri, rd in enumerate(rebal):
        ...
        for code in elig:
            ...
            if not ma_rising(cl):
                continue
            s = compute_score(cl, hi, lo, vol_norm=vol_norm)
            if s > 0:
...
def run_all_variants(label, vol_norm):
    """Run all three backtest variants with the given vol_norm setting."""
    print(f"\n{'=' * 70}")
    print(f"  {label}")
    print(f"{'=' * 70}")

    # Progressive
    print(f"\n  Progressive enrollment backtest ...")
    t1 = time.time()
    r_prog = run_backtest(progressive=True, vol_norm=vol_norm)
    dt1 = time.time() - t1
    print(f"    Done in {dt1:.1f}s")
    print(f"    CAGR: {r_prog['cagr']:+.2f}%  Sharpe: {r_prog['sharpe']:.3f}  MaxDD: {r_prog['maxdd']:.2f}%")

    # Static all-in
    print(f"\n  Static all-in backtest ...")
    t2 = time.time()
    r_static = run_backtest(progressive=False, vol_norm=vol_norm)
    dt2 = time.time() - t2
    print(f"    Done in {dt2:.1f}s")
    print(f"    CAGR: {r_static['cagr']:+.2f}%  Sharpe: {r_static['sharpe']:.3f}  MaxDD: {r_static['maxdd']:.2f}%")

    # 6-ETF baseline
    print(f"\n  6-ETF baseline backtest ...")
    ...
    for s in baseline_syms:
        CATEGORY_MAP[s] = "all"
    t3 = time.time()
    r_baseline = run_backtest(progressive=False, symbols_override=baseline_syms, vol_norm=vol_norm)
    dt3 = time.time() - t3
    CATEGORY_MAP.clear()
    ...
    return r_prog, r_static, r_baseline


if __name__ == "__main__":
    t0 = time.time()
    print("=" * 70)
    print("MULTI-INDUSTRY ETF ROTATION BACKTEST — VOLATILITY NORMALIZATION TEST")
    print("=" * 70)
    ...
    print(f"VOL_NORM = {VOL_NORM}")
    print()

    # Run with VOL_NORM = True
    rp_on, rs_on, rb_on = run_all_variants(
        f"VOL-NORM ON  (ADX / max(50, vol*100)  ―  Phase 2.2 validation)",
        vol_norm=True,
    )

    # Run with VOL_NORM = False (original behavior, for comparison)
    rp_off, rs_off, rb_off = run_all_variants(
        f"VOL-NORM OFF (ADX / 50.0 fixed  ―  original baseline)",
        vol_norm=False,
    )

    # === COMPARISON SUMMARY ===
    print("\n" + "=" * 70)
    print("VOLATILITY NORMALIZATION COMPARISON (ON vs OFF)")
    print("=" * 70)

    for variant_label, r_on, r_off in [
        ("Progressive (25 ETF)", rp_on, rp_off),
        ("Static (25 ETF)",       rs_on, rs_off),
        ("Baseline (6 ETF)",      rb_on, rb_off),
    ]:
        print(f"\n  --- {variant_label} ---")
        for key, name, fmt_str in [
            ("cagr", "CAGR %", "+.2f"),
            ("sharpe", "Sharpe", ".3f"),
            ("maxdd", "MaxDD %", ".2f"),
            ("calmar", "Calmar", ".3f"),
            ...
            voff = r_off[key]
            delta = von - voff
            print(f"    {name:<10s}  ON={von:{fmt_str}}  OFF={voff:{fmt_str}}  Δ={delta:+.2f}")

    # Qualitative assessment: which ETFs get selected more/less often?
    print("\n" + "=" * 70)
    print("QUALITATIVE ASSESSMENT")
    print("=" * 70)
    print()
    print("  Vol-norm raises the ADX denominator for high-vol ETFs,")
    print("  reducing their ADX score contribution. Low-vol ETFs (bonds,")
    print("  gold) see a smaller denominator penalty, making them more")
    print("  competitive. This should reduce sector dominance by tech/")
    print("  semiconductor/new-energy ETFs.")
    print()
    print("  Key indicators:")
    print(f"    - Progressive CAGR: ON={rp_on['cagr']:+.2f}%  OFF={rp_off['cagr']:+.2f}%  "
          f"Δ={rp_on['cagr'] - rp_off['cagr']:+.2f}%")
    print(f"    - Progressive Sharpe: ON={rp_on['sharpe']:.3f}  OFF={rp_off['sharpe']:.3f}  "
          f"Δ={rp_on['sharpe'] - rp_off['sharpe']:+.3f}")
    print(f"    - Progressive MaxDD: ON={rp_on['maxdd']:.2f}%  OFF={rp_off['maxdd']:.2f}%  "
          f"Δ={rp_on['maxdd'] - rp_off['maxdd']:+.2f}%")
    print()
    print(f"  Look-ahead bias (static - progressive):")
    print(f"    ON: {rs_on['cagr'] - rp_on['cagr']:+.2f}% CAGR")
    print(f"    OFF: {rs_off['cagr'] - rp_off['cagr']:+.2f}% CAGR")
    print()
    print(f"  Multi-industry vs Baseline delta:")
    print(f"    ON: {rp_on['cagr'] - rb_on['cagr']:+.2f}% CAGR")
    print(f"    OFF: {rp_off['cagr'] - rb_off['cagr']:+.2f}% CAGR")
